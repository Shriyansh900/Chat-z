...
export default function FriendsPanel({ open, onClose }: FriendsPanelProps) {
  ...
  const { chats, setChats, setActiveChat, setMessages } = useChatStore();
  ...
  const handleOpenDM = async (friend: User) => {
    ...
    try {
      // POST /chats creates or returns existing 1-on-1 chat
      const res = await api.post('/chats', { userId: friend._id });
      const chat = res.data;
      setChats([chat, ...chats.filter((c) => c._id !== chat._id)]);
      setActiveChat(chat);
      const msgs = await api.get(`/messages/${chat._id}`);
      setMessages(
        Array.isArray(msgs.data) ? msgs.data : msgs.data ? [msgs.data] : [],
      );
      onClose();
    } catch {
      // silently fail
